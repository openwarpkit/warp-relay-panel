...
## Тестирование, фаззинг и бенчмарки

Для обеспечения надежности и производительности горячих путей в проекте предусмотрены тесты:

```bash
make test               # Запуск стандартных unit-тестов
make test-bench         # Запуск бенчмарков (проверка отсутствия аллокаций)
make test-fuzz          # Запуск fuzz-тестов на устойчивость к повреждению файлов
```

Фаззинг-тестирование (`FuzzRefcountLoad`, `FuzzRatelimitLoad`, `FuzzTrafficLoad`) проверяет поведение агента при чтении битых/неполных JSON-файлов состояния на диске, предотвращая падения при сбоях питания сервера.

## Оптимизации производительности (v2.2.0+)

1. **Zero-Reflection Sorting**: `sort.Slice` заменен на `slices.Sort` и `slices.SortFunc` (Go 1.21+ generics). Это полностью убрало оверхед на рефлексию и снизило нагрузку на CPU в воркере при сортировке больших списков клиентов.
2. **Zero-Allocation JSON Streaming**: Запись файлов состояния и HTTP-ответы переведены с прожорливого `json.MarshalIndent` на потоковый стриминг через `json.NewEncoder` напрямую в дескрипторы файлов/HTTP-соединения. Это защищает агент от OOM при десятках тысяч активных IP.
3. **Double Buffering & Atomic State**: Для delta-аккумуляции трафика conntrack применен пинг-понг буфер с атомарным свапом указателей, исключающий взаимную блокировку (lock contention) читающих и пишущих потоков.
