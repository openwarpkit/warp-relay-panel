...
PREBUILT_BIN="${GO_AGENT_DIR}/dist/warp-relay-agent-min"
...
if [ "$USE_PREBUILT" = "1" ]; then
    echo -e "${Y}[5/8] Установка prebuilt-бинаря...${N}"
    cp "$PREBUILT_BIN" ${INSTALL_DIR}/warp-relay-agent-min
else
    echo -e "${Y}[5/8] Сборка warp-relay-agent-min (1-3 мин, нужно >500 MB RAM)...${N}"
    cd "${GO_AGENT_DIR}"
    make tidy
    make build-min
